import { geoProjectionMutator, geoProjection, geoAzimuthalEquidistantRaw, geoAzimuthalEqualAreaRaw, geoCentroid, geoRotation, geoEquirectangular, geoOrthographic, geoCircle, geoStream, geoBounds, geoInterpolate, geoGnomonic, geoGnomonicRaw } from 'd3-geo';
...
function august() {
  return geoProjection(augustRaw)
      .scale(66.1603);
}
...
function bertin() {
  ...
  return geoProjection(bertin1953Raw())
    .rotate([-16.5, -42])
    .scale(176.57)
    .center([7.93, 0.09]);
}
...
function bottomley() {
  var sinPsi = 0.5,
      m = geoProjectionMutator(bottomleyRaw),
      p = m(sinPsi);
...
function chamberlin(p0, p1, p2) { // TODO order matters!
  var c = geoCentroid({type: "MultiPoint", coordinates: [p0, p1, p2]}),
      R = [-c[0], -c[1]],
      r = geoRotation(R),
      f = chamberlinRaw(pointRadians(r(p0)), pointRadians(r(p1)), pointRadians(r(p2)));
  f.invert = solve2d(f);
  var p = geoProjection(f).rotate(R),
      center = p.center;
...
function collignon() {
  return geoProjection(collignonRaw)
      .scale(95.6464)
      .center([0, 30]);
}
...
function eckert2() {
  return geoProjection(eckert2Raw)
      .scale(165.664);
}
...
function eckert3() {
  return geoProjection(eckert3Raw)
      .scale(180.739);
}
...
function eckert4() {
  return geoProjection(eckert4Raw)
      .scale(180.739);
}
...
function eckert6() {
  return geoProjection(eckert6Raw)
      .scale(173.044);
}
...
function eisenlohr() {
  return geoProjection(eisenlohrRaw)
      .scale(62.5271);
}
...
function foucaut() {
  return geoProjection(foucautRaw)
      .scale(135.264);
}
...
function foucautSinusoidal() {
  var alpha = 0.5,
      m = geoProjectionMutator(foucautSinusoidalRaw),
      p = m(alpha);
...
function gingery() {
  var n = 6,
      ...
      cRho = cos(rho),
      sRho = sin(rho),
      m = geoProjectionMutator(gingeryRaw),
      p = m(rho, n),
      stream_ = p.stream,
      epsilon = 1e-2,
      cr = -cos(epsilon * radians),
...
function ginzburg5() {
  return geoProjection(ginzburg5Raw)
      .scale(153.93);
}
...
function ginzburg8() {
  return geoProjection(ginzburg8Raw)
      .scale(131.747);
}
...
function ginzburg9() {
  return geoProjection(ginzburg9Raw)
      .scale(131.087);
}
...
function gringorten() {
  return geoProjection(squareRaw(gringortenRaw))
      .scale(239.75);
}
...
function guyou() {
  return geoProjection(squareRaw(guyouRaw))
      .scale(151.496);
}
...
function hammerRetroazimuthal() {
  var phi0 = 0,
      m = geoProjectionMutator(hammerRetroazimuthalRaw),
      p = m(phi0),
      rotate_ = p.rotate,
      stream_ = p.stream,
      circle = geoCircle();
...
function healpix() {
  var H = 4,
      m = geoProjectionMutator(healpixRaw),
      p = m(H),
      stream_ = p.stream;
...
function hill() {
  var K = 1,
      m = geoProjectionMutator(hillRaw),
      p = m(K);
...
function hufnagel() {
  var a = 1,
    ...
    psiMax = 45 * radians,
    ratio = 2,
    mutate = geoProjectionMutator(hufnagelRaw),
...
function hyperelliptical() {
  var alpha = 0,
      ...
      m = geoProjectionMutator(hyperellipticalRaw),
      p = m(alpha, k, gamma);
...
function lagrange() {
  var n = 0.5,
      m = geoProjectionMutator(lagrangeRaw),
      p = m(n);
...
function larrivee() {
  return geoProjection(larriveeRaw)
      .scale(97.2672);
}
...
function modifiedStereographic(coefficients, rotate) {
  var p = geoProjection(modifiedStereographicRaw(coefficients)).rotate(rotate).clipAngle(90),
      r = geoRotation(rotate),
      center = p.center;
...
function naturalEarth2() {
  return geoProjection(naturalEarth2Raw)
      .scale(175.295);
}
...
function nicolosi() {
  return geoProjection(nicolosiRaw)
    .scale(127.267);
}
...
function patterson() {
  return geoProjection(pattersonRaw)
      .scale(139.319);
}
...
function polyconic() {
  return geoProjection(polyconicRaw)
      .scale(103.74);
}
...
function polyhedral(root, face) {
  ...
  var proj = geoProjection(forward),
      stream_ = proj.stream;
...
function outline(stream, node, parent) {
  var point,
      ...
      b = geoBounds({type: "MultiPoint", coordinates: notPoles}),
      inside = false,
      j = -1,
      dx = b[1][0] - b[0][0];
  // TODO
  ...
  for (var i = 0; i < n; ++i) {
    ...
    if (Array.isArray(edge)) {
      if (!inside) {
        stream.point((point = geoInterpolate(edge[0], c)(epsilon))[0], point[1]);
        inside = true;
      }
      stream.point((point = geoInterpolate(edge[1], c)(epsilon))[0], point[1]);
    } else {
      inside = false;
...
function butterfly(faceProjection) {
  ...
  faceProjection = faceProjection || function(face) {
    ...
    return geoGnomonic().scale(1).translate([0, 0]).rotate([-c[0], -c[1]]);
  };
...
function collignon$1(faceProjection) {
  ...
  faceProjection = faceProjection || function(face) {
    var c = geoCentroid({type: "MultiPoint", coordinates: face});
    return geoProjection(collignonK).translate([0, 0]).scale(1).rotate(c[1] > 0 ? [-c[0], 0] : [180 - c[0], 180]);
  };
...
function waterman(faceProjection) {
  ...
  faceProjection = faceProjection || function(face) {
    var c = face.length === 6 ? geoCentroid({type: "MultiPoint", coordinates: face}) : face[0];
    return geoGnomonic().scale(1).translate([0, 0]).rotate([-c[0], -c[1]]);
  };
...
function quincuncial(project) {
  ...
  return geoProjection(projectQuincuncial)
      .rotate([-90, -90, 45])
      .clipAngle(180 - 1e-3);
}
...
function twoPoint(raw, p0, p1) {
  var i = geoInterpolate(p0, p1),
      o = i(0.5),
      a = geoRotation([-o[0], -o[1]])(p0),
      b = i.distance / 2,
      y = -asin(sin(a[1] * radians) / sin(b)),
      R = [-o[0], -o[1], -(a[0] > 0 ? pi - y : y) * degrees],
      p = geoProjection(raw(b)).rotate(R),
      r = geoRotation(R),
      center = p.center;
...
function twoPointAzimuthalRaw(d) {
  ...
  function forward(lambda, phi) {
    var coordinates = geoGnomonicRaw(lambda, phi);
    coordinates[0] *= cosd;
    return coordinates;
  }

  forward.invert = function(x, y) {
    return geoGnomonicRaw.invert(x / cosd, y);
  };
...
function twoPointEquidistantRaw(z0) {
  if (!(z0 *= 2)) return geoAzimuthalEquidistantRaw;
  var lambdaa = -z0 / 2,
      lambdab = -lambdaa,
      z02 = z0 * z0,
...
function vanDerGrinten() {
  return geoProjection(vanDerGrintenRaw)
      .scale(79.4183);
}
...
function vanDerGrinten2() {
  return geoProjection(vanDerGrinten2Raw)
      .scale(79.4183);
}
...
function vanDerGrinten3() {
  return geoProjection(vanDerGrinten3Raw)
        .scale(79.4183);
}
...
function vanDerGrinten4() {
  return geoProjection(vanDerGrinten4Raw)
      .scale(127.16);
}
...
function wagner() {
  ...
  var poleline = 65 * radians,
      ...
      inflation = 20,
      ratio = 200,
      mutate = geoProjectionMutator(wagnerRaw),
...
function wiechel() {
  return geoProjection(wiechelRaw)
      .rotate([0, -90, 45])
      .scale(124.75)
      .clipAngle(180 - 1e-3);
}
