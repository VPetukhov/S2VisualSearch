import { hasOwnProperty, isNumber, isString, writeConfig, splitAccessPath, stringValue, isObject, isBoolean as isBoolean$1, isArray, array, logger, Warn, isFunction, mergeConfig, identity } from 'vega-util';
import 'array-flat-polyfill';
import clone_ from 'clone';
import deepEqual_ from 'fast-deep-equal';
import stableStringify from 'fast-json-stable-stringify';
import { isObject as isObject$1, writeConfig as writeConfig$1, isArray as isArray$1, isString as isString$1 } from 'vega';
import { parseSelector } from 'vega-event-selector';
import { parseExpression as parseExpression$1 } from 'vega-expression';

const name = "vega-lite";
const author = "Dominik Moritz, Kanit \"Ham\" Wongsuphasawat, Arvind Satyanarayan, Jeffrey Heer";
const version = "5.2.0";
const collaborators = [
...
const description = "Vega-Lite is a concise high-level language for interactive visualization.";
const main = "build/vega-lite.js";
const unpkg = "build/vega-lite.min.js";
const jsdelivr = "build/vega-lite.min.js";
const module = "build/src/index";
...
function extractTitleConfig(titleConfig) {
    ...
    rest = __rest$1(titleConfig, ["anchor", "frame", "offset", "orient", "angle", "limit", "color", "subtitleColor", "subtitleFont", "subtitleFontSize", "subtitleFontStyle", "subtitleFontWeight", "subtitleLineHeight", "subtitlePadding"]);
...
function signalRefOrValue(value) {
    if (isExprRef(value)) {
        const { expr } = value, rest = __rest$2(value, ["expr"]);
...
function timeUnitToString(tu) {
    const _a = normalizeTimeUnit(tu), { utc } = _a, rest = __rest$3(_a, ["utc"]);
    if (rest.unit) {
        return ((utc ? 'utc' : '') +
            keys(rest)
...
function isFieldRange(range) {
    return isObject$1(range) && 'field' in range;
}
...
const NON_TYPE_DOMAIN_RANGE_VEGA_SCALE_PROPERTY_INDEX = __rest$4(SCALE_PROPERTY_INDEX, ["type", "domain", "range", "rangeMax", "rangeMin", "scheme"]);
...
function channelCompatibility(fieldDef, channel) {
    ...
    switch (channel) {
        ...
        case SHAPE:
        case STROKEDASH:
            if (!isDiscrete$1(fieldDef) && !isDiscretizing(fieldDef)) {
                return {
                    compatible: false,
...
function partLayerMixins(markDef, part, compositeMarkConfig, partBaseSpec) {
    ...
    if (markDef[part] || (markDef[part] === undefined && compositeMarkConfig[part])) {
        return [
            Object.assign(Object.assign({}, partBaseSpec), { mark: Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({}, compositeMarkConfig[part]), (clip ? { clip } : {})), (color ? { color } : {})), (opacity ? { opacity } : {})), (isMarkDef(partBaseSpec.mark) ? partBaseSpec.mark : { type: partBaseSpec.mark })), { style: `${mark}-${part}` }), (isBoolean$1(markDef[part]) ? {} : markDef[part])) })
        ];
    }
    return [];
}
...
function normalizeBoxPlot(spec, { config }) {
    ...
    const { mark, encoding: _encoding, params, projection: _p } = spec, outerSpec = __rest$8(spec, ["mark", "encoding", "params", "projection"]);
...
function errorBarParams(spec, compositeMark, config) {
    var _a;
    // TODO: use selection
    const { mark, encoding, params, projection: _p } = spec, outerSpec = __rest$9(spec, ["mark", "encoding", "params", "projection"]);
    const markDef = isMarkDef(mark) ? mark : { type: mark };
    // TODO(https://github.com/vega/vega-lite/issues/3702): add selection support
    if (params) {
...
function initConfig(specifiedConfig = {}) {
    const { color, font, fontSize, selection } = specifiedConfig, restConfig = __rest$b(specifiedConfig, ["color", "font", "fontSize", "selection"]);
    ...
    if (selection) {
        writeConfig$1(mergedConfig, 'selection', selection, true);
    }
...
class PathOverlayNormalizer {
    ...
    run(spec, normParams, normalize) {
        ...
        const { params, projection, mark, encoding: e } = spec, outerSpec = __rest$d(spec, ["params", "projection", "mark", "encoding"]);
    ...
    if (isFieldOrDatumDef(channelDef)) {
        ...
        if (hasConditionalFieldOrDatumDef(channelDef)) {
            ...
            if (fd) {
                ...
                const channelDefWithoutCondition = __rest$e(channelDef, ["condition"]);
...
class CoreNormalizer extends SpecMapper {
    ...
    mapLayerRepeat(spec, params) {
        const { repeat, spec: childSpec } = spec, rest = __rest$f(spec, ["repeat", "spec"]);
    ...
    mapNonLayerRepeat(spec, params) {
        var _a;
        const { repeat, spec: childSpec, data } = spec, remainingProperties = __rest$f(spec, ["repeat", "spec", "data"]);
    ...
    mapFacetedUnit(spec, normParams) {
        ...
        const _a = spec.encoding, { row, column, facet } = _a, encoding = __rest$f(_a, ["row", "column", "facet"]);
        // Mark and encoding should be moved into the inner spec
        const { mark, width, projection, height, view, params, encoding: _ } = spec, outerSpec = __rest$f(spec, ["mark", "width", "projection", "height", "view", "params", "encoding"]);
    ...
    getFacetMappingAndLayout(facets, params) {
        ...
        if (row || column) {
            ...
            for (const channel of [ROW, COLUMN]) {
                ...
                if (def) {
                    const defWithoutLayout = __rest$f(def, ["align", "center", "spacing", "columns"]);
            ...
            const { align, center, spacing, columns } = facet, facetMapping = __rest$f(facet, ["align", "center", "spacing", "columns"]);
            return {
    ...
    mapLayer(spec, _a) {
        ...
        var { parentEncoding, parentProjection } = _a, otherParams = __rest$f(_a, ["parentEncoding", "parentProjection"]);
        const { encoding, projection } = spec, rest = __rest$f(spec, ["encoding", "projection"]);
    ...
    mapUnit(spec, normParams) {
        ...
        if (selection) {
            return Object.assign(Object.assign({}, rest), { params: entries(selection).map(([name, selDef]) => {
                    var _a;
                    const _b = selDef, { init: value, bind, empty } = _b, select = __rest$g(_b, ["init", "bind", "empty"]);
...
function normalizeTransforms(spec, normParams) {
    const { transform: tx } = spec, rest = __rest$g(spec, ["transform"]);
    if (tx) {
        const transform = tx.map((t) => {
            if (isFilter(t)) {
                return { filter: normalizePredicate$1(t, normParams) };
            }
            else if (isBin(t) && isBinParams(t.bin)) {
                return Object.assign(Object.assign({}, t), { bin: normalizeBinExtent(t.bin) });
            }
            else if (isLookup(t)) {
                const _a = t.from, { selection: param } = _a, from = __rest$g(_a, ["selection"]);
                return param
...
function normalizeChannelDef(obj, normParams) {
    ...
    if (isScaleFieldDef(enc) && ((_b = (_a = enc.scale) === null || _a === void 0 ? void 0 : _a.domain) === null || _b === void 0 ? void 0 : _b.selection)) {
        const _c = enc.scale.domain, { selection: param } = _c, domain = __rest$g(_c, ["selection"]);
    ...
    if (isConditionalDef(enc)) {
        if (isArray$1(enc.condition)) {
            enc.condition = enc.condition.map((c) => {
                const { selection, param, test } = c, cond = __rest$g(c, ["selection", "param", "test"]);
                return param ? c : Object.assign(Object.assign({}, cond), { test: normalizePredicate$1(c, normParams) });
            });
        }
        else {
            const _d = normalizeChannelDef(enc.condition, normParams), { selection, param, test } = _d, cond = __rest$g(_d, ["selection", "param", "test"]);
            enc.condition = param
                ? enc.condition
                : Object.assign(Object.assign({}, cond), { test: normalizePredicate$1(enc.condition, normParams) });
        }
    }
    return enc;
}
function normalizeBinExtent(bin) {
    ...
    if (ext === null || ext === void 0 ? void 0 : ext.selection) {
        const { selection: param } = ext, rest = __rest$g(ext, ["selection"]);
        ...
        for (const selection of selections) {
            ...
            if (!selection.views || !selection.views.length) {
                ...
                for (const view of selection.views) {
                    ...
                    if ((isString$1(view) && (view === spec.name || path.indexOf(view) >= 0)) ||
                        (isArray$1(view) &&
    ...
    for (const selCmpt of vals((_a = model.component.selection) !== null && _a !== void 0 ? _a : {})) {
        ...
        if (selCmpt.init) {
            const fields = selCmpt.project.items.map(proj => {
                const rest = __rest$h(proj, ["signals"]);
                return rest;
            });
...
class TimeUnitNode extends DataFlowNode {
    ...
    static makeFromTransform(parent, t) {
        const _a = Object.assign({}, t), { timeUnit } = _a, other = __rest$i(_a, ["timeUnit"]);
    ...
    assemble() {
        ...
        for (const f of vals(this.formula)) {
            ...
            const _a = normalizeTimeUnit(timeUnit), { unit, utc } = _a, params = __rest$i(_a, ["unit", "utc"]);
        ...
        return hasSignal.length > 0
            ...
            : allSignals.concat({
                ...
                value: selCmpt.project.items.map(proj => {
                    const rest = __rest$j(proj, ["signals", "hasLegend"]);
...
function disableDirectManipulation(selCmpt, selDef) {
    if (isString$1(selDef.select) || !selDef.select.on)
        delete selCmpt.events;
    if (isString$1(selDef.select) || !selDef.select.clear)
        delete selCmpt.clear;
    if (isString$1(selDef.select) || !selDef.select.toggle)
...
function getDependentFields(expression) {
    const ast = parseExpression$1(expression);
...
function assembleAxis(axisCmpt, kind, config, opt = { header: false }) {
    var _a, _b;
    const _c = axisCmpt.combine(), { disable, orient, scale, labelExpr, title, zindex } = _c, axis = __rest$l(_c, ["disable", "orient", "scale", "labelExpr", "title", "zindex"]);
    if (disable) {
        return undefined;
    }
    for (const prop in axis) {
        ...
        if (propType && propType !== kind && propType !== 'both') {
            ...
            const { condition } = propValue, valueOrSignalRef = __rest$l(propValue, ["condition"]);
            const conditions = array(condition);
            ...
            if (propIndex) {
                ...
                const vgRef = [
                    ...conditions.map(c => {
                        const { test } = c, valueOrSignalCRef = __rest$l(c, ["test"]);
...
function assembleLegend(legendCmpt, config) {
    var _a, _b, _c;
    const _d = legendCmpt.combine(), { disable, labelExpr, selections } = _d, legend = __rest$m(_d, ["disable", "labelExpr", "selections"]);
    if (disable) {
        return undefined;
    }
...
class BinNode extends DataFlowNode {
    ...
    assemble() {
        return vals(this.bins).flatMap(bin => {
            ...
            const _a = bin.bin, { extent } = _a, params = __rest$n(_a, ["extent"]);
...
class Optimizer {
    constructor() {
        ...
        __classPrivateFieldSet$1(this, _Optimizer_modified, false, "f");
    }
    // Once true, #modified is never set to false
    setModified() {
        __classPrivateFieldSet$1(this, _Optimizer_modified, true, "f");
    }
    get modifiedFlag() {
        return __classPrivateFieldGet$1(this, _Optimizer_modified, "f");
    }
}
...
function mergeDomains(domains) {
    const uniqueDomains = unique(domains.map(domain => {
        ...
        if (isDataRefDomain(domain)) {
            const domainWithoutSort = __rest$o(domain, ["sort"]);
...
function assembleScalesForModel(model) {
    return keys(model.component.scales).reduce((scales, channel) => {
        ...
        const { name, type, selectionExtent, domains: _d, range: _r, reverse } = scale, otherScaleProps = __rest$p(scale, ["name", "type", "selectionExtent", "domains", "range", "reverse"]);
...
function scaleType(specifiedScale, channel, fieldDef, mark, hasNestedOffsetScale = false) {
    const defaultScaleType = defaultType$2(channel, fieldDef, mark, hasNestedOffsetScale);
...
function defaultType$2(channel, fieldDef, mark, hasNestedOffsetScale) {
    var _a;
...
class Model {
    ...
    assembleEncodeFromView(view) {
        // Exclude "style"
        const baseView = __rest$q(view, ["style"]);
        const e = {};
    ...
    assembleLayout() {
        ...
        const _a = this.layout, { spacing } = _a, layout = __rest$q(_a, ["spacing"]);
    ...
    assembleTitle() {
        var _a, _b, _c;
        const _d = (_a = this.title) !== null && _a !== void 0 ? _a : {}, { encoding } = _d, titleNoEncoding = __rest$q(_d, ["encoding"]);
...
class DensityTransformNode extends DataFlowNode {
    ...
    assemble() {
        const _a = this.transform, { density } = _a, rest = __rest$r(_a, ["density"]);
...
class LoessTransformNode extends DataFlowNode {
    ...
    assemble() {
        const _a = this.transform, { loess, on } = _a, rest = __rest$s(_a, ["loess", "on"]);
...
class QuantileTransformNode extends DataFlowNode {
    ...
    assemble() {
        const _a = this.transform, { quantile } = _a, rest = __rest$t(_a, ["quantile"]);
...
class RegressionTransformNode extends DataFlowNode {
    ...
    assemble() {
        const _a = this.transform, { regression, on } = _a, rest = __rest$u(_a, ["regression", "on"]);
...
const image = {
    ...
    encodeEntry: (model) => {
        return Object.assign(Object.assign(Object.assign(Object.assign({}, baseEncodeEntry(model, {
            ...
            size: 'ignore',
            theta: 'ignore'
        })), rectPosition(model, 'x')), rectPosition(model, 'y')), text(model, 'url'));
    }
};
...
function assembleTopLevelModel(model, topLevelProperties, datasets = {}, usermeta) {
    ...
    const { params } = topLevelProperties, otherTopLevelProps = __rest$v(topLevelProperties, ["params"]);
...
export { accessPathDepth, accessPathWithDatum, compile, contains, deepEqual, deleteNestedProperty, duplicate, entries, every, fieldIntersection, flatAccessWithDatum, getFirstDefined, hasIntersection, hash, internalField, isBoolean, isEmpty, isEqual, isInternalField, isNullOrFalse, isNumeric, keys, logicalExpr, mergeDeep, never, normalize, normalizeAngle, omit, pick, prefixGenerator, removePathFromField, replaceAll, replacePathInField, resetIdCounter, setEqual, some, stringify, titleCase, unique, uniqueId, vals, varName, version$1 as version };
